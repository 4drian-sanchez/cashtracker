...
describe('budget middleware- validateBudgetExists', () => {
    it('Should handle non-existent budget', async () => {
        ...
        expect(data).toEqual({error: 'El presupuesto no a sido encontrado'})
        expect(next).not.toHaveBeenCalled()
    })

    it('should proceed to next middleware if budget exists', async () => {
        //mockResolvedValue: Valida solo la parte del try
    ...
    it('should handle non-existent budget', async () => {
        //mockRejectedValue: Valida solo la parte del catch
